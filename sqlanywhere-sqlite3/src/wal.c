...
int sqlanywhere_pager_codec(sqlanywhere_pghdr *p, void **ret);

typedef sqlanywhere_pghdr PgHdr;
...
# error "SEH is not supported in SqlAnywhere due to virtual WAL backward compatibility!"
#else
# define SEH_TRY          
# define SEH_EXCEPT(X)    
...
#ifdef SQLANYWHERE_CHECKPOINT_ONLY_FULL
    // in case of SQLANYWHERE_CHECKPOINT_ONLY_FULL option we want to either checkpoint whole WAL or quickly abort the checkpoint
...
static int walWriteOneFrame(
  ...
  pData = pPage->pData;
  rc = sqlanywhere_pager_codec(pPage, &pData);
  if( rc ) return rc;
    ...
    ** checksums must be recomputed when the transaction is committed.  */
    if( iFirst && (p->pDirty || isCommit==0) ){
      ...
      if( iWrite>=iFirst ){
        ...
        rc = sqlanywhere_pager_codec(p, &pData);
        if( rc ) return rc;
  ...
  if( rc==SQLITE_OK ){
    ...
    */
    if( eMode!=SQLITE_CHECKPOINT_PASSIVE ){
...
#ifndef SQLANYWHERE_DISABLE_CHECKPOINT_DOWNGRADE
      if( rc==SQLITE_OK ){
        pWal->writeLock = 1;
      }else if( rc==SQLITE_BUSY ){
      ...
      // SQLANYWHERE_DISABLE_CHECKPOINT_DOWNGRADE compile time option disables downgrade of checkpoint mode
      if( rc==SQLITE_OK ){
        pWal->writeLock = 1;
      }
#endif
...
static int sqlanywhereWalPathnameLen(int n) {
  return n ? n + 4 : 0;
}

static void sqlanywhereGetWalPathname(char *buf, const char *orig, int orig_len) {
...
static int sqlite3WalOpen(
  ...
  sqlanywhere_wal *out                     /* OUT: Allocated Wal handle */
){
  int rc;                         /* Return Code */
  ...
  */
  ...
  if( rc!=SQLITE_OK ){
    ...
    out->methods.xFrames = (int (*)(wal_impl *, int, sqlanywhere_pghdr *, unsigned int, int, int, int *))sqlite3WalFrames;
...
const sqlanywhere_wal_manager sqlite3_wal_manager = { 
    .pData = NULL, 
    .xOpen = (int (*)(wal_manager_impl *, sqlite3_vfs *, sqlite3_file *, int, long long, const char*, sqlanywhere_wal *))sqlite3WalOpen,
