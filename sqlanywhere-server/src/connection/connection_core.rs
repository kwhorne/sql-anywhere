...
/// The base connection type, shared between legacy and sqlanywhere-wal implementations
pub(super) struct CoreConnection<W> {
    conn: sqlanywhere_sys::Connection<W>,
    stats: Arc<Stats>,
...
impl<W: Wal + Send + 'static> CoreConnection<W> {
    ...
    pub(super) fn raw_mut(&mut self) -> &mut sqlanywhere_sys::Connection<W> {
        &mut self.conn
    }

    pub(super) fn raw(&self) -> &sqlanywhere_sys::Connection<W> {
        &self.conn
    }
    ...
    pub(super) fn run<B: QueryResultBuilder>(
        ...
        while !vm.finished() {
            ...
            if has_timeout {
                ...
                vm.builder().step_error(Error::SqlAnywhereTxTimeout)?;
    ...
    pub(super) fn checkpoint(&self) -> Result<()> {
        ...
        histogram!("sqlanywhere_server_wal_checkpoint_time", start.elapsed());
        Ok(())
    }
...
mod test {
    ...
    #[test]
    fn test_sqlanywhere_conn_builder_driver() {
        test_driver(1000, |b| {
