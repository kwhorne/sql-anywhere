...
impl FromStr for Cipher {
    type Err = sqlanywhere_ffi::Error;
...
extern "C" {
    ...
    fn sqlite3mc_config(
        db: *mut sqlanywhere_ffi::sqlite3,
...
pub fn pghdr_creator(
    data: &mut [u8; 4096],
    _db: *mut sqlanywhere_ffi::sqlite3,
) -> sqlanywhere_ffi::sqlanywhere_pghdr {
    #[cfg(feature = "encryption")]
...
pub unsafe fn set_encryption_cipher(db: *mut sqlanywhere_ffi::sqlite3, cipher_id: i32) -> i32 {
...
/// # Safety
/// db must point to a vaid sqlite database
pub unsafe fn set_encryption_key(db: *mut sqlanywhere_ffi::sqlite3, key: &[u8]) -> i32 {
...
pub fn reset_encryption_key(db: *mut sqlanywhere_ffi::sqlite3, key: &[u8]) -> i32 {
    unsafe { sqlite3_rekey(db, key.as_ptr() as _, key.len() as _) as i32 }
}

#[cfg(feature = "encryption")]
pub fn leak_pager(db: *mut sqlanywhere_ffi::sqlite3) -> *mut crate::ffi::Pager {
    unsafe { sqlanywhere_leak_pager(db) }
}

#[cfg(feature = "encryption")]
pub fn generate_initial_vector(seed: u32, iv: &mut [u8]) {
    unsafe { sqlanywhere_generate_initial_vector(seed, iv.as_mut_ptr()) }
}

#[cfg(feature = "encryption")]
pub fn generate_aes256_key(user_password: &[u8], digest: &mut [u8]) {
    unsafe {
        sqlanywhere_generate_aes256_key(
...
impl<W: Wal> Connection<W> {
        ...
        T: WalManager<Wal = W>,
    {
        ...
        let conn = unsafe {
            ...
            let mut rc = sqlanywhere_ffi::sqlanywhere_open_v3(
                path.as_ptr(),
                &mut conn as *mut _,
                flags,
                vfs,
            ...
            if let Some(cfg) = encryption_config {
                ...
                if set_encryption_key(conn, &cfg.encryption_key) != sqlanywhere_ffi::SQLITE_OK {
                    return Err(Error::Bug("failed to set encryption key"));
                }
            }

            if rc == 0 {
                rc = sqlanywhere_ffi::sqlite3_wal_autocheckpoint(conn, auto_checkpoint as _);
            }

            if rc != 0 {
                sqlanywhere_ffi::sqlite3_close(conn);
    ...
    pub unsafe fn handle(&self) -> *mut sqlanywhere_ffi::sqlite3 {
        #[cfg(feature = "rusqlite")]
        {
            self.conn.handle()
        }
    ...
    pub fn db_change_counter(&self) -> Result<u32, std::ffi::c_int> {
        let mut file_ptr: *mut sqlanywhere_ffi::sqlite3_file = std::ptr::null_mut();
        let rc = unsafe {
            sqlanywhere_ffi::sqlite3_file_control(
                self.handle(),
                "main\0".as_ptr() as *const _,
                sqlanywhere_ffi::SQLITE_FCNTL_FILE_POINTER,
                &mut file_ptr as *mut _ as *mut _,
            )
        };
        if rc != sqlanywhere_ffi::SQLITE_OK {
            return Err(rc);
        }
        let counter = unsafe {
    ...
    fn reserved_bytes(&self, reserve: Option<i32>) -> Result<i32, std::ffi::c_int> {
        ...
        let rc = unsafe {
            sqlanywhere_ffi::sqlite3_file_control(
                self.handle(),
                "main\0".as_ptr() as *const _,
                sqlanywhere_ffi::SQLITE_FCNTL_RESERVE_BYTES,
...
//             // that the reference must outlive the connection. This is guaranteed by the marker in
//             // the returned connection.
//             let rc = crate::ffi::sqlanywhere_open_v2(
//                 filename.as_ptr(),
//                 &mut conn as *mut _,
//                 flags,
//                 std::ptr::null_mut(),
