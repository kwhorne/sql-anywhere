...
impl SqliteInjector {
    pub async fn new(
        ...
        capacity: usize,
        auto_checkpoint: u32,
        encryption_config: Option<sqlanywhere_sys::EncryptionConfig>,
    ) -> super::Result<Self> {
...
pub(in super::super) struct SqliteInjectorInner {
    ...
    connection: Arc<Mutex<sqlanywhere_sys::Connection<InjectorWal>>>,
...
impl SqliteInjectorInner {
    ...
    /// Flush the buffer to sqlanywhere WAL.
    ...
    fn try_flush(&mut self) -> Result<Option<FrameNo>, Error> {
        ...
        let mut stmt =
            connection.prepare_cached("INSERT INTO sqlanywhere_temp_injection VALUES (42)")?;
        ...
        match stmt.execute(()).and_then(|_| connection.cache_flush()) {
            ...
            Err(e) => {
                if let Some(err) = e.sqlite_error() {
                    if err.extended_code == SQLANYWHERE_INJECT_OK {
                        // refresh schema
                    ...
                    } else if err.extended_code == SQLANYWHERE_INJECT_OK_TXN {
                        self.is_txn = true;
                        assert!(self.buffer.lock().is_empty());
                        return Ok(None);
                    } else if err.extended_code == SQLANYWHERE_INJECT_FATAL {
    ...
    fn begin_txn(&mut self) -> Result<(), Error> {
...

        {
            ...
            let new_conn = sqlanywhere_sys::Connection::open(
                &self.path,
                OpenFlags::SQLITE_OPEN_READ_WRITE
                    | OpenFlags::SQLITE_OPEN_CREATE
                    | OpenFlags::SQLITE_OPEN_URI
        ...
        let mut stmt =
            conn.prepare_cached("CREATE TABLE IF NOT EXISTS sqlanywhere_temp_injection (x)")?;
        stmt.execute(())?;
