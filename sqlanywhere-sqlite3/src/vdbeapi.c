...
*/
void sqlanywhere_stmt_interrupt(sqlite3_stmt *pStmt){
...
int sqlite3_stmt_status(sqlite3_stmt *pStmt, int op, int resetFlag){
...
#ifdef SQLITE_ENABLE_API_ARMOR
  if( !pStmt 
   || (op!=SQLITE_STMTSTATUS_MEMUSED && (op<0||(op>=ArraySize(pVdbe->aCounter)&&op<SQLANYWHERE_STMTSTATUS_BASE)))
  ){
    (void)SQLITE_MISUSE_BKPT;
    return 0;
  }
  ...
  if( op==SQLITE_STMTSTATUS_MEMUSED ){
    ...
    sqlite3_mutex_leave(db->mutex);
  }else if( op>=SQLANYWHERE_STMTSTATUS_BASE ){
    v = pVdbe->aSqlanywhereCounter[op - SQLANYWHERE_STMTSTATUS_BASE];
    if( resetFlag ) pVdbe->aSqlanywhereCounter[op - SQLANYWHERE_STMTSTATUS_BASE] = 0;
  }else{
    v = pVdbe->aCounter[op];
