...
/* SqlAnywhere extension: pager codec */

#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
int sqlanywhere_pager_codec_impl(sqlanywhere_pghdr *hdr, void **ret);
int sqlanywhere_db_has_codec(sqlite3_vfs* pVfs, const char* zFilename);
#endif

int sqlanywhere_pager_has_codec(struct Pager *_p) {
#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
  return sqlanywhere_db_has_codec(_p->pVfs, _p->zFilename);
#else
  return 0;
#endif
}

int sqlanywhere_pager_codec(sqlanywhere_pghdr *hdr, void **ret) {
  if (!ret) {
	return SQLITE_MISUSE_BKPT;
  }
#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
  return sqlanywhere_pager_codec_impl(hdr, ret);
#else
  *ret = hdr->pData;
  return SQLITE_OK;
#endif
}
/* end of SqlAnywhere extension: pager codec */
...
*/
sqlanywhere_wal_methods *sqlite3PagerWalMethods(Pager *pPager){
...
static int pagerOpenWal(Pager *pPager){
  ...
  */
  ...
  if( rc==SQLITE_OK ){
    sqlanywhere_wal *wal = (sqlanywhere_wal*)sqlite3MallocZero(sizeof(sqlanywhere_wal));
    if (!wal) {
        rc = SQLITE_NOMEM;
    }
    if (rc == SQLITE_OK) {
