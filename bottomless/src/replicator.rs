...
impl Options {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        let db_id = env_var("SQLANYWHERE_BOTTOMLESS_DATABASE_ID").ok();
        let aws_endpoint = env_var("SQLANYWHERE_BOTTOMLESS_ENDPOINT").ok();
        let bucket_name = env_var_or("SQLANYWHERE_BOTTOMLESS_BUCKET", "bottomless");
        let max_batch_interval = Duration::from_secs(
            env_var_or("SQLANYWHERE_BOTTOMLESS_BATCH_INTERVAL_SECS", 15).parse::<u64>()?,
        );
        let access_key_id = env_var("SQLANYWHERE_BOTTOMLESS_AWS_ACCESS_KEY_ID").ok();
        let secret_access_key = env_var("SQLANYWHERE_BOTTOMLESS_AWS_SECRET_ACCESS_KEY").ok();
        let session_token = env_var("SQLANYWHERE_BOTTOMLESS_AWS_SESSION_TOKEN").ok();
        let region = env_var("SQLANYWHERE_BOTTOMLESS_AWS_DEFAULT_REGION").ok();
        let max_frames_per_batch =
            env_var_or("SQLANYWHERE_BOTTOMLESS_BATCH_MAX_FRAMES", 10000).parse::<usize>()?;
        let s3_max_parallelism =
            env_var_or("SQLANYWHERE_BOTTOMLESS_S3_PARALLEL_MAX", 32).parse::<usize>()?;
        let use_compression =
            CompressionKind::parse(&env_var_or("SQLANYWHERE_BOTTOMLESS_COMPRESSION", "zstd"))
                .map_err(|e| anyhow!("unknown compression kind: {}", e))?;
        let encryption_cipher = env_var("SQLANYWHERE_BOTTOMLESS_ENCRYPTION_CIPHER").ok();
        let encryption_key = env_var("SQLANYWHERE_BOTTOMLESS_ENCRYPTION_KEY")
            .map(Bytes::from)
            .ok();
        let verify_crc = match env_var_or("SQLANYWHERE_BOTTOMLESS_VERIFY_CRC", true)
            .to_lowercase()
            .as_ref()
        {
            "yes" | "true" | "1" | "y" | "t" => true,
            ...
            other => bail!(
                "Invalid SQLANYWHERE_BOTTOMLESS_VERIFY_CRC environment variable: {}",
                other
            ),
        };
        let skip_snapshot = match env_var_or("SQLANYWHERE_BOTTOMLESS_SKIP_SNAPSHOT", false)
            .to_lowercase()
            .as_ref()
        {
            "yes" | "true" | "1" | "y" | "t" => true,
            "no" | "false" | "0" | "n" | "f" => false,
            other => bail!(
                "Invalid SQLANYWHERE_BOTTOMLESS_SKIP_SNAPSHOT environment variable: {}",
                other
            ),
        };
        ...
        let skip_shutdown_upload =
            env_var_or("SQLANYWHERE_BOTTOMLESS_SKIP_SHUTDOWN_UPLOAD", false).parse::<bool>()?;
...
impl Replicator {
    ...
    // Opens a raw SqlAnywhere connection that doesn't checkpoint.
    // Useful for reading metadata from the database file.
    fn open_db(&self) -> Result<sqlanywhere_sys::Connection<sqlanywhere_sys::wal::Sqlite3Wal>> {
        use sqlanywhere_sys::connection::OpenFlags;
        use sqlanywhere_sys::wal::Sqlite3WalManager;
        let flags = OpenFlags::SQLITE_OPEN_READ_WRITE
            | OpenFlags::SQLITE_OPEN_CREATE
            | OpenFlags::SQLITE_OPEN_URI
            | OpenFlags::SQLITE_OPEN_NO_MUTEX;

        let conn = sqlanywhere_sys::Connection::open(
            &self.db_path,
            flags,
            Sqlite3WalManager::new(),
            sqlanywhere_sys::connection::NO_AUTOCHECKPOINT, // no checkpointing
    ...
    fn read_change_counter(&self) -> Result<[u8; 4]> {
        ...
        if std::env::var("SQLANYWHERE_BOTTOMLESS_DISABLE_INIT_CHECKPOINTING").is_ok()
            || std::env::var("SQLANYWHERE_DISABLE_INIT_CHECKPOINTING").is_ok()
        {
            std::mem::forget(conn);
        }
    ...
    async fn restore_wal(
        ...
        let mut injector = sqlanywhere_replication::injector::SqliteInjector::new(
            db_path.to_path_buf(),
            4096,
            sqlanywhere_sys::connection::NO_AUTOCHECKPOINT,
            encryption_config,
        )
        .await?;
        ...
        'restore_wal: loop {
            ...
            for obj in objs {
                ...
                while let Some(frame) = reader.next_frame_header().await? {
                    ...
                    let frame_to_inject = sqlanywhere_replication::frame::Frame::from_parts(
                        &sqlanywhere_replication::frame::FrameHeader {
