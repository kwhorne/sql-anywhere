    ...
    async fn push_with_retry(
        ...
        loop {
            ...
            if let Some(remote_encryption) = &self.remote_encryption {
                req = req.header("x-elyra-encryption-key", remote_encryption.key.as_string());
            }
...
pub async fn try_pull(
    ...
    // sqlanywhere maintain consistent state about WAL sync session locally in the insert_handle
