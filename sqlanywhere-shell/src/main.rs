...
#[derive(Debug, Parser)]
#[command(name = "sqlanywhere")]
#[command(about = "SqlAnywhere client", long_about = None)]
struct Cli {
    #[clap()]
...
impl Shell {
    fn new(args: Cli) -> Result<Self> {
        ...
        Ok(Self {
            ...
            main_prompt: "sqlanywhere> ".to_string(),
...
"#,
    ...
    // Presents SqlAnywhere values in human-readable form
...
fn main() -> Result<()> {
    ...
    history.push(".sqlanywhere_history");
    rl.load_history(history.as_path()).ok();

    println!("SqlAnywhere version 0.2.0");
