...
use sqlanywhere_replication::rpc::replication::BoxReplicationService;
...
const SQLANYWHERE_PAGE_SIZE: u64 = 4096;
    ...
    D: Connector,
{
    ...
    fn init_sqlite_globals(&self) {
        if let Some(soft_limit_mb) = self.db_config.soft_heap_limit_mb {
            ...
            unsafe {
                sqlanywhere_sys::ffi::sqlite3_soft_heap_limit64(soft_limit_mb as i64 * 1024 * 1024)
            };
        }
        if let Some(hard_limit_mb) = self.db_config.hard_heap_limit_mb {
            tracing::warn!("Setting hard heap limit to {hard_limit_mb}MiB");
            unsafe {
                sqlanywhere_sys::ffi::sqlite3_hard_heap_limit64(hard_limit_mb as i64 * 1024 * 1024)
            };
        }
    }
    ...
    pub async fn start(mut self) -> anyhow::Result<()> {
        ...
        if std::env::var("SQLANYWHERE_SQLITE_MIMALLOC").is_ok() {
            setup_sqlite_alloc();
        }

        INIT.call_once(|| {
            if let Ok(size) = std::env::var("SQLANYWHERE_EXPERIMENTAL_PAGER") {
