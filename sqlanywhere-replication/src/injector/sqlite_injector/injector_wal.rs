...
use sqlanywhere_sys::ffi::{Pager, PgHdr};
...
pub const SQLANYWHERE_INJECT_FATAL: c_int = 200;
/// Injection succeeded, left on a open txn state
pub const SQLANYWHERE_INJECT_OK_TXN: c_int = 201;
/// Injection succeeded
pub const SQLANYWHERE_INJECT_OK: c_int = 202;
    ...
    fn insert_frames(
...

        {
            ...
            if let Err(e) = self.inner.insert_frames(
                ...
                return Err(sqlanywhere_sys::wal::Error::new(SQLANYWHERE_INJECT_FATAL));
            }
        ...
        if !self.is_txn {
            Err(sqlanywhere_sys::wal::Error::new(SQLANYWHERE_INJECT_OK))
        } else {
            Err(sqlanywhere_sys::wal::Error::new(SQLANYWHERE_INJECT_OK_TXN))
        }
    }
