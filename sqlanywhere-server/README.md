# `sqld` - a server mode for SQL Anywhere

The `sqld` ("SQL daemon") project is a server mode for
[SQL Anywhere](https://github.com/sqlanywhere/sqlanywhere/).

Embedded SQL databases such as SQL Anywhere and SQLite are great for a lot of use
...
Run the command below to run all tests for `sqlanywhere` and `sqlanywhere-server`.
...
```bash
SQLANYWHERE_BOTTOMLESS_BUCKET=my-bucket                 # Default bucket name: bottomless
SQLANYWHERE_BOTTOMLESS_ENDPOINT='http://localhost:9000' # address can be overridden for local testing, e.g. with Minio
SQLANYWHERE_BOTTOMLESS_AWS_SECRET_ACCESS_KEY=           # regular AWS variables are used
SQLANYWHERE_BOTTOMLESS_AWS_ACCESS_KEY_ID=               # ... to set up auth, regions, etc.
SQLANYWHERE_BOTTOMLESS_AWS_REGION=                      # .
```
