use sqlanywhere_wasi::{instantiate, new_linker, Error, Result};
...
fn main() -> Result<()> {
    ...
    let (mut store, instance) = instantiate(&linker, "../../sqlanywhere.wasm")?;
    ...
    let sqlanywhere_wasi_init = instance.get_typed_func::<(), ()>(&mut store, "sqlanywhere_wasi_init")?;
    let open_func = instance.get_typed_func::<i32, i32>(&mut store, "sqlanywhere_wasi_open_db")?;
    let exec_func = instance.get_typed_func::<(i32, i32), i32>(&mut store, "sqlanywhere_wasi_exec")?;
    let close_func = instance.get_typed_func::<i32, i32>(&mut store, "sqlite3_close")?;

    sqlanywhere_wasi_init.call(&mut store, ())?;
