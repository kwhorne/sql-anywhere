...
static void sync_db(sqlite3 *db_primary, sqlite3 *db_backup){
  ...
  ensure(sqlanywhere_wal_frame_count(db_primary, &max_frame) == SQLITE_OK, "can't get frame count: %s\n", sqlite3_errmsg(db_primary));
  ensure(sqlanywhere_wal_insert_begin(db_backup) == SQLITE_OK, "can't begin commit: %s\n", sqlite3_errmsg(db_backup));
  for(int i=1; i<=max_frame; i++){
    char frame[4096+24];
    ensure(sqlanywhere_wal_get_frame(db_primary, i, frame, sizeof(frame)) == SQLITE_OK, "can't get frame: %s\n", sqlite3_errmsg(db_primary));
    int conflict;
    ensure(sqlanywhere_wal_insert_frame(db_backup, i, frame, sizeof(frame), &conflict) == SQLITE_OK, "can't inject frame: %s\n", sqlite3_errmsg(db_backup));
    ensure(conflict == 0, "conflict at frame %d\n", i);
  }
  ensure(sqlanywhere_wal_insert_end(db_backup) == SQLITE_OK, "can't end commit: %s\n", sqlite3_errmsg(db_backup));
}
...
void test_huge_payload() {
    ...
    ensure(sqlanywhere_wal_frame_count(db_backup, &max_frame) == SQLITE_OK, "failed to get frames count: %s\n", sqlite3_errmsg(db_backup));
    assert(max_frame == 0);
...
void test_conflict() {    
  ...
  unsigned int max_frame;
  ensure(sqlanywhere_wal_frame_count(db2, &max_frame) == SQLITE_OK, "can't get frame count: %s\n", sqlite3_errmsg(db2));
  ensure(sqlanywhere_wal_insert_begin(db_synced) == SQLITE_OK, "can't begin commit: %s\n", sqlite3_errmsg(db_synced));
  // First 3 frames should not conflict.
  for(int i=1; i<=3; i++){
    char frame[4096+24];
    ensure(sqlanywhere_wal_get_frame(db2, i, frame, sizeof(frame)) == SQLITE_OK, "can't get frame: %s\n", sqlite3_errmsg(db2));
    int conflict;
    ensure(sqlanywhere_wal_insert_frame(db_synced, i, frame, sizeof(frame), &conflict) == SQLITE_OK, "conflict detected: %s\n", sqlite3_errmsg(db_synced));
    ensure(conflict == 0, "conflict at frame %d\n", i);
  }
  ...
  for(int i=4; i<=max_frame; i++){
    ...
    int conflict;
    ensure(sqlanywhere_wal_insert_frame(db_synced, i, frame, sizeof(frame), &conflict) == SQLITE_ERROR, "conflict not detected: %s\n", sqlite3_errmsg(db_synced));
    ensure(conflict == 1, "no conflict at frame %d\n", i);
  }
  ensure(sqlanywhere_wal_insert_end(db_synced) == SQLITE_OK, "can't end commit: %s\n", sqlite3_errmsg(db_synced));
}
