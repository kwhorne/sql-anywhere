// Example of using a remote sync server with sqlanywhere.
...
async fn main() {
    ...
    let db_path = std::env::var("SQLANYWHERE_DB_PATH")
        .map_err(|_| {
            eprintln!(
                "Please set the SQLANYWHERE_DB_PATH environment variable to set to local database path."
            )
        })
        .unwrap();

    // The remote sync URL to use.
    let sync_url = std::env::var("SQLANYWHERE_SYNC_URL")
        .map_err(|_| {
            eprintln!(
                "Please set the SQLANYWHERE_SYNC_URL environment variable to set to remote sync URL."
            )
        })
        .unwrap();

    let namespace = std::env::var("SQLANYWHERE_NAMESPACE").ok();
