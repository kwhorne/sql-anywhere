...
use sqlanywhere_server::config::{AdminApiConfig, DbConfig, RpcClientConfig, RpcServerConfig};
...
fn apply_partial_snapshot() {
    ...
    sim.client("client", async move {
        let primary = sqlanywhere::Database::open_remote_with_connector(
            "http://primary:8080",
            "",
            TurmoilConnector,
        )
...
fn replica_interactive_transaction() {
    ...
    sim.client("client", async move {
        ...
        let tx = conn
            .transaction_with_behavior(sqlanywhere::TransactionBehavior::Immediate)
            .await
            .unwrap();
        ...
        // sqlanywhere-client doesn't support read your writes yet
