...
impl Rows {
    ...
    pub fn next(&self) -> Result<Option<Row>> {
        ...
        match err {
            sqlanywhere_sys::ffi::SQLITE_OK => Ok(None),
            sqlanywhere_sys::ffi::SQLITE_DONE => Ok(None),
            sqlanywhere_sys::ffi::SQLITE_ROW => Ok(Some(Row {
    ...
    pub fn column_type(&self, idx: i32) -> Result<ValueType> {
        ...
        match val {
            sqlanywhere_sys::ffi::SQLITE_INTEGER => Ok(ValueType::Integer),
            sqlanywhere_sys::ffi::SQLITE_FLOAT => Ok(ValueType::Real),
            sqlanywhere_sys::ffi::SQLITE_BLOB => Ok(ValueType::Blob),
            sqlanywhere_sys::ffi::SQLITE_TEXT => Ok(ValueType::Text),
            sqlanywhere_sys::ffi::SQLITE_NULL => Ok(ValueType::Null),
...
pub trait FromValue {
    fn from_sql(val: sqlanywhere_sys::Value) -> Result<Self>
    where
        Self: Sized;
}
