...
#[cfg(feature = "core")]
pub use sqlanywhere_sys::{Cipher, EncryptionConfig};
...
cfg_core! {
    bitflags::bitflags! {
        /// Flags that can be passed to sqlanywhere to open a database in specific
        /// modes.
        #[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
        #[repr(C)]
        pub struct OpenFlags: ::std::os::raw::c_int {
            const SQLITE_OPEN_READ_ONLY = sqlanywhere_sys::ffi::SQLITE_OPEN_READONLY;
            const SQLITE_OPEN_READ_WRITE = sqlanywhere_sys::ffi::SQLITE_OPEN_READWRITE;
            const SQLITE_OPEN_CREATE = sqlanywhere_sys::ffi::SQLITE_OPEN_CREATE;
        }
    }
...
cfg_core! {
    impl Database {
        /// Open an in-memory sqlanywhere database.
...
cfg_remote! {
    impl Database {
        /// Open a remote based HTTP database using sqlanywhere's hrana protocol.
        #[deprecated = "Use the new `Builder` to construct `Database`"]
        ...
        /// Connect to a remote sqlanywhere using sqlanywhere's hrana protocol with a custom connector.
        #[deprecated = "Use the new `Builder` to construct `Database`"]
...
impl Database {
    ...
    ///     sqlanywhere C ffi and create a connection to the sqlanywhere database.
    ...
    pub fn connect(&self) -> Result<Connection> {
        match &self.db_type {
            #[cfg(feature = "core")]
            DbType::Memory { db } => {
                use crate::local::impls::SqlanywhereConnection;

                let conn = db.connect()?;

                let conn = std::sync::Arc::new(SqlanywhereConnection { conn });
            ...
            DbType::Sync {
                ...
                let local = SqlanywhereConnection { conn };
            ...
            DbType::Offline {
                ...
                use crate::{
                    hrana::connection::HttpConnection, local::impls::SqlanywhereConnection,
                ...
                let conn = std::sync::Arc::new(SqlanywhereConnection { conn: local });
                Ok(Connection { conn })
            }
