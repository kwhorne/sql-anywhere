...
use sqlanywhere_replication::rpc::replication::{
...
impl ReplicationLogService {
    ...
    async fn authenticate<T>(
        ...
        if self.service_internal && req.metadata().get("sqlanywhere-proxied").is_some()
...
fn map_frame_stream_output(
    r: Result<(sqlanywhere_replication::frame::Frame, Option<DateTime<Utc>>), LogReadError>,
) -> Result<Frame, Status> {
    match r {
...
impl ReplicationLog for ReplicationLogService {
    ...
    async fn log_entries(
        ...
        if let WalFlavor::Sqlanywhere = req.get_ref().wal_flavor() {
            return Err(Status::invalid_argument("sqlanywhere wal not supported"));
        }
...
mod snapshot_stream {
    ...
    use sqlanywhere_replication::frame::FrameNo;
    use sqlanywhere_replication::rpc::replication::Frame;
            ...
            while let Some(frame) = frames.next().await {
                match frame {
                    Ok(mut frame) => {
                        ...
                        yield Ok(Frame {
                            data: sqlanywhere_replication::frame::Frame::from(frame).bytes(),
                            timestamp: None,
                            durable_frame_no: None,
                        });
                    }
