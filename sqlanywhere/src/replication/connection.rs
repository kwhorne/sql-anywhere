...
pub struct RemoteConnection {
    pub(self) local: SqlanywhereConnection,
    ...
    async fn execute_transactional_batch(&self, sql: &str) -> Result<BatchRows> {
        ...
        steps.push(Step {
            query: Some(Query {
                ...
                params: Some(sqlanywhere_replication::rpc::proxy::query::Params::Positional(
                    Positional::default(),
                )),
                ..Default::default()
            }),
        ...
        for (idx, stmt) in stmts.into_iter().enumerate() {
            let step = Step {
                cond: Some(Cond {
                    cond: Some(sqlanywhere_replication::rpc::proxy::cond::Cond::Ok(OkCond {
                        step: idx as i64,
                        ..Default::default()
                    })),
                }),
        ...
        steps.push(Step {
            cond: Some(Cond {
                cond: Some(sqlanywhere_replication::rpc::proxy::cond::Cond::Not(Box::new(
                    NotCond {
...
impl From<sqlanywhere_replication::rpc::proxy::Column> for ColumnMeta {
    fn from(col: sqlanywhere_replication::rpc::proxy::Column) -> Self {
        Self {
            name: col.name.clone(),
            origin_name: None,
            table_name: None,
...
#[derive(Debug)]
struct RemoteRow(Vec<Value>, Vec<sqlanywhere_replication::rpc::proxy::Column>);
