use sqlanywhere::{Builder, Cipher, EncryptionConfig, Value};
...
async fn main() {
    ...
    let auth_token = std::env::var("SQLANYWHERE_AUTH_TOKEN").unwrap_or_else(|_| {
        println!("Using empty token since SQLANYWHERE_TOKEN was not set");
        "".to_string()
    });

    let url = std::env::var("SQLANYWHERE_URL")
        .unwrap_or_else(|_| {
            println!("Using http://localhost:8080 SQLANYWHERE_URL was not set");
            "http://localhost:8080".to_string()
        })
        .replace("sqlanywhere", "https");
