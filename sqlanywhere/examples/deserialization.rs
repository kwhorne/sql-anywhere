use sqlanywhere::{de, Builder};

#[tokio::main]
async fn main() {
    let db = if let Ok(url) = std::env::var("SQLANYWHERE_URL") {
        let token = std::env::var("SQLANYWHERE_AUTH_TOKEN").unwrap_or_else(|_| {
            println!("SQLANYWHERE_TOKEN not set, using empty token...");
            "".to_string()
        });
