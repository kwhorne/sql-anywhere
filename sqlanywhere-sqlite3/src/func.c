...
void run_wasm(sqlite3_context *context, int argc, sqlite3_value **argv) {
  static sqlanywhere_wasm_udf_api api = {
    .sqlanywhere_value_type = sqlite3_value_type,
    .sqlanywhere_value_int = sqlite3_value_int,
    .sqlanywhere_value_double = sqlite3_value_double,
    .sqlanywhere_value_text = sqlite3_value_text,
    .sqlanywhere_value_blob = sqlite3_value_blob,
    .sqlanywhere_value_bytes = sqlite3_value_bytes,
    .sqlanywhere_result_error = sqlite3_result_error,
    .sqlanywhere_result_error_nomem = sqlite3_result_error_nomem,
    .sqlanywhere_result_int = sqlite3_result_int,
    .sqlanywhere_result_double = sqlite3_result_double,
    .sqlanywhere_result_text = sqlite3_result_text,
    .sqlanywhere_result_blob = sqlite3_result_blob,
    .sqlanywhere_result_null = sqlite3_result_null,
    .sqlanywhere_malloc = sqlite3_malloc,
    .sqlanywhere_free = sqlite3_free
  };
  void *user_data = sqlite3_user_data(context);
  sqlanywhere_wasm_module_t *module = *(sqlanywhere_wasm_module_t **)(user_data);
  const char *func_name = (const char *)user_data + sizeof(module);
  sqlanywhere_run_wasm(&api, context, context->pVdbe->db->wasm.engine, module, func_name, argc, argv);
}
...
FuncDef *try_instantiate_wasm_function(sqlite3 *db, const char *pName, int nName, const char *pSrcBody, int nBody, int nArg, char **err_msg_buf) {
  ...
  if (!db->wasm.engine) {
    db->wasm.engine = sqlanywhere_wasm_engine_new();
  }

  sqlanywhere_wasm_module_t *module = sqlanywhere_compile_wasm_module(db->wasm.engine, pSrcBody, nBody, sqlite3MallocZero, err_msg_buf);
  if (!module) {
    return NULL;
  }
  ...
  if(!pBest->u.pDestructor) {
    sqlite3OomFault(db);
    sqlanywhere_free_wasm_module(pBest);
    return NULL;
  }
  pBest->u.pDestructor->nRef = 1;
  pBest->u.pDestructor->xDestroy = sqlanywhere_free_wasm_module;
...
int sqlanywhere_try_initialize_wasm_func_table(sqlite3 *db) {
#ifdef SQLITE_TEST
  // Tcl tests assume there are no extra tables during their execution
  return SQLITE_OK;
#endif
  int rc = sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS sqlanywhere_wasm_func_table (name text PRIMARY KEY, body text) WITHOUT ROWID", NULL, NULL, NULL);
  if (rc == SQLITE_OK) {
    // If the table exists, register all the functions
    sqlite3_stmt *stmt;
    rc = sqlite3_prepare_v2(db, "SELECT name, body FROM sqlanywhere_wasm_func_table", -1, &stmt, 0);
