...
executed simple statement: 'CREATE INDEX t_idx ON t ( sqlanywhere_vector_idx(emb) );'
...
It is linked against libsqlanywhere.so which resides in the `../libs/` directory and must be explicitly built from `sqlanywhere-sqlite3` sources:
```sh
$> basename $(pwd)
sqlanywhere-sqlite3
