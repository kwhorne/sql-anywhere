...
int sqlanywhere_handle_extra_attach_params(sqlite3* db, const char* zName, const char* zPath, sqlite3_value* pKey, char** zErrDyn);
#endif

#ifdef SQLANYWHERE_ENCRYPTION
...
#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
    pNew->hasCodec = sqlanywhere_db_has_codec(pVfs, zPath);
#else
    pNew->hasCodec = 0;
#endif
...
#ifdef SQLANYWHERE_EXTRA_URI_PARAMS
  if (rc == SQLITE_OK) {
    rc = sqlanywhere_handle_extra_attach_params(db, zName, zPath, argv, &zErrDyn);
  }
#endif
