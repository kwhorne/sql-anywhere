...
use sqlanywhere_server::net::AddrIncoming;
use sqlanywhere_server::version::Version;
use sqlanywhere_server::Server;
...
struct Cli {
    ...
    /// Soft heap size limit in mebibytes - SqlAnywhere will try to not go over this limit with memory usage.
    #[clap(long, env = "SQLD_SOFT_HEAP_LIMIT_MB")]
    soft_heap_limit_mb: Option<usize>,

    /// Hard heap size limit in mebibytes - SqlAnywhere will bail out with SQLITE_NOMEM error
    ...
    #[clap(
        long,
        env = "SQLANYWHERE_STORAGE_SERVER_ADDR",
        default_value = "http://0.0.0.0:5002"
    )]
    storage_server_address: String,

    /// Enable bottomless to sqlanywhere_wal migration. Bottomless replication must be enabled.
    #[clap(
        long,
        env = "SQLANYWHERE_MIGRATE_BOTTOMLESS",
    ...
    #[clap(
        long,
        env = "SQLANYWHERE_SYNC_FROM_STORAGE",
        requires = "enable_bottomless_replication"
    )]
    sync_from_storage: bool,
    /// Whether to force loading all WAL at startup, with sqlanywhere-wal
    ...
    #[clap(
        long,
        env = "SQLANYWHERE_SYNC_CONCCURENCY",
    ...
    #[clap(long, env = "SQLANYWHERE_DISABLE_METRICS")]
...
fn enable_sqlanywhere_logging() {
    use std::ffi::c_int;
    use std::sync::Once;
    static ONCE: Once = Once::new();

    fn sqlanywhere_log(code: c_int, msg: &str) {
        tracing::error!("sqlite error {code}: {msg}");
    }

    ONCE.call_once(|| unsafe {
        rusqlite::trace::config_log(Some(sqlanywhere_log)).unwrap();
    });
}
    ...
    if let Some(ref subcommand) = args.subcommand {
        match subcommand {
            UtilsSubcommands::AdminShell {
                ...
                let client = sqlanywhere_server::admin_shell::AdminShellClient::new(
                    admin_api_url.clone(),
                    auth.clone(),
                );
