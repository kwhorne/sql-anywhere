...
pub fn setup_migration_table(conn: &mut rusqlite::Connection) -> Result<(), Error> {
    static TASKS_TABLE_QUERY: Lazy<String> = Lazy::new(|| {
        format!(
            "CREATE TABLE IF NOT EXISTS sqlite3_sqlanywhere_tasks (
...
pub fn has_pending_migration_task(conn: &rusqlite::Connection) -> Result<bool, Error> {
    Ok(conn.query_row(
        "SELECT COUNT(1) FROM sqlite3_sqlanywhere_tasks WHERE finished = false",
        (),
...
pub fn enqueue_migration_task(
    ...
    conn.execute(
        "INSERT OR IGNORE INTO sqlite3_sqlanywhere_tasks (job_id, status, migration) VALUES (?, ?, ?)",
...
pub fn abort_migration_task(conn: &rusqlite::Connection, job_id: i64) -> Result<(), Error> {
    ...
    conn.execute("INSERT OR REPLACE INTO sqlite3_sqlanywhere_tasks (job_id, status, error, migration) VALUES (?, ?, ?, ?)",
...
pub fn step_migration_task_run(conn: &rusqlite::Connection, job_id: i64) -> Result<(), Error> {
    conn.execute(
        "
            UPDATE sqlite3_sqlanywhere_tasks
            SET status = ?
...
fn get_task_infos(
    ...
    Ok(conn.query_row(
        "SELECT status, migration, error FROM sqlite3_sqlanywhere_tasks WHERE job_id = ?",
        [job_id],
        |row| {
...
pub(super) fn update_db_task_status(
    ...
    conn.execute(
        "UPDATE sqlite3_sqlanywhere_tasks SET status = ?, error = ? WHERE job_id = ?",
