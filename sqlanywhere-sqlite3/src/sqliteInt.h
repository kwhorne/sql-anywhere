...
// NOTICE: SqlAnywhere extension: disabled WAL also implies we don't want shared memory via mmap
...
// SEH is not supported in SqlAnywhere.
// /*
...
#if defined(SQLANYWHERE_ENABLE_WASM_RUNTIME) && !defined(SQLITE_AMALGAMATION)
...
typedef struct sqlanywhere_wasm_ctx {
  sqlanywhere_wasm_engine_t *engine;
} sqlanywhere_wasm_ctx;
#endif
...
#endif
#ifdef SQLANYWHERE_ENABLE_WASM_RUNTIME
  sqlanywhere_wasm_ctx wasm;        /* WebAssembly runtime context */
#endif
#ifndef SQLITE_OMIT_WAL
...
#define SQLANYWHERE_RANDOM_ROWID_MARKER 0xffffffff
...
struct KeyInfo {
  /*
   * zIndexName is used in OP_OpenVectorIdx op-code and required for SqlAnywhere
...
void sqlanywhereAlterAlterColumn(Parse*, SrcList*, Token*, Token*, int);
