...
`sqld` is a server mode for [SQL Anywhere](https://sqlanywhere.org), which provides SQLite interface and dialect for use cases such as edge functions where it's impractical to embed a full database engine.
...
The client provides a SQLite ABI compatible interface as a drop-in replacement for applications using SQL Anywhere or SQLite. The client library transforms SQLite C API calls into PostgreSQL wire protocol messages and sends them to the primary server.

The primary server is a `sqld` process, which servers SQLite dialect over the PostgreSQL wire protocol. The server can either be backed by single-node `SQL Anywhere` database or by a mvSQLite backend, which provides improved write concurrency, high availability, and fault tolerance using FoundationDB.

Replica servers is a `sqld` process, which only serves reads locally, and delegates writes to the primary server. The server is backed by a `SQL Anywhere` database.
...
The primary server either performs the write against its local `SQL Anywhere` database or processes it via `mvSQLite`, which uses FoundationDB.
