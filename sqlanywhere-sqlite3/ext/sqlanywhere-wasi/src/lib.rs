...
pub fn instantiate(
    linker: &Linker<State>,
    sqlanywhere_wasm_path: impl AsRef<std::path::Path>,
    ...
    let sqlanywhere_module = Module::from_file(linker.engine(), sqlanywhere_wasm_path.as_ref())?;

    let mut store = Store::new(linker.engine(), wasi_ctx);
    let instance = linker.instantiate(&mut store, &sqlanywhere_module)?;
