...
impl HttpSender {
    pub fn new(
        ...
        let version = HeaderValue::try_from(format!("sqlanywhere-remote-{ver}")).unwrap();
    ...
    async fn send(
        ...
        let mut req_builder = hyper::Request::post(url.as_ref())
            ...
            .header("x-sqlanywhere-client-version", self.version.clone());
        ...
        if let Some(remote_encryption) = &self.remote_encryption {
            req_builder =
                req_builder.header("x-elyra-encryption-key", remote_encryption.key.as_string());
        }
