...
mod tests {
    extern "C" {
        fn sqlanywhere_close_hook(
            db: *mut sqlanywhere_sys::ffi::sqlite3,
            callback: Option<
                extern "C" fn(arg: *mut std::ffi::c_void, db: *mut sqlanywhere_sys::ffi::sqlite3),
            >,
    ...
    use sqlanywhere_sys::rusqlite::Connection;
    ...
    fn get_read_written(conn: &Connection, stmt: &str) -> (i32, i32) {
        ...
        drop(rows);
        let mut rows_read = sqlanywhere_sys::rusqlite::StatementStatus::FullscanStep;
        let mut rows_written = sqlanywhere_sys::rusqlite::StatementStatus::FullscanStep;
    ...
    fn test_close_hook() {
        ...
        extern "C" fn cb(closed: *mut c_void, _db: *mut sqlanywhere_sys::ffi::sqlite3) {
            unsafe {
        ...
        unsafe {
            sqlanywhere_close_hook(conn.handle(), Some(cb), &mut closed as *mut _ as *mut _);
        }
        assert!(!closed);
        drop(conn);
        assert!(closed);
