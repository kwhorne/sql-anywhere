...
const char sqlanywhere_version[] = SQLANYWHERE_VERSION;
#endif
...
const char *sqlanywhere_libversion(void){ return SQLANYWHERE_VERSION; }
...
static int sqlite3Close(sqlite3 *db, int forceZombie){
  ...
  */
  ...
  if (db->wasm.engine) {
    sqlanywhere_wasm_engine_free(db->wasm.engine);
  }
#endif
...
**/
int sqlanywhere_wal_disable_checkpoint(sqlite3 *db) {
...
*/
int sqlanywhere_wal_frame_count(
  sqlite3* db,
...
** Begin a WAL commit.
*/
int sqlanywhere_wal_insert_begin(sqlite3 *db) {
  Pager *pPager;
  int rc;
...
int sqlanywhere_wal_insert_end(sqlite3 *db) {
  Pager *pPager;
  int rc;
...
*/
int sqlanywhere_wal_insert_frame(
  sqlite3* db,
  unsigned int iFrame,
  void *pBuf,
  unsigned int nBuf,
...
#ifdef SQLANYWHERE_PRE_VFS_HOOK
void sqlanywhere_pre_vfs_hook(const char *zVfs);
#endif
...
#ifdef SQLANYWHERE_EXTRA_URI_PARAMS
int sqlanywhere_handle_extra_uri_params(sqlite3 *db, const char *zOpen);
#endif

#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
int sqlanywhere_db_has_codec(sqlite3_vfs* pVfs, const char* zFilename);
#endif
...
#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
  db->aDb[0].hasCodec = sqlanywhere_db_has_codec(db->pVfs, zOpen);
#else
  db->aDb[0].hasCodec = 0;
#endif
...
#ifdef SQLANYWHERE_EXTRA_URI_PARAMS
  if (rc == SQLITE_OK) {
    rc = sqlanywhere_handle_extra_uri_params(db, zOpen);
  }
#endif
...
int sqlanywhere_open_v3(
  const char *filename,   /* Database filename (UTF-8) */
  sqlite3 **ppDb,         /* OUT: SQLite db handle */
  int flags,              /* Flags */
  const char *zVfs,       /* Name of VFS module to use, NULL for default */
  sqlanywhere_wal_manager wal_manager   /* wal_manager implemetation */
  ) {
...
int sqlite3_file_control(sqlite3 *db, const char *zDbName, int op, void *pArg){
  ...
  if( pBtree ){
    ...
    if( op==SQLITE_FCNTL_FILE_POINTER ){
      ...
      *(sqlanywhere_wal_methods**)pArg = sqlite3PagerWalMethods(pPager);
      rc = SQLITE_OK;
#endif
