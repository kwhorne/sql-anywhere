...
pub struct Statement {
    ...
    pub(crate) inner: Arc<sqlanywhere_sys::Statement>,
    sql: String,
}
...
impl Statement {
    ...
    pub(crate) fn prepare(
        conn: Connection,
        raw: *mut sqlanywhere_sys::ffi::sqlite3,
        sql: &str,
    ) -> Result<Statement> {
        match unsafe { sqlanywhere_sys::prepare_stmt(raw, sql) } {
            Ok(stmt) => Ok(Statement {
                conn,
                inner: Arc::new(stmt),
                sql: sql.to_string(),
            }),
            Err(sqlanywhere_sys::Error::LibError(_err)) => Err(Error::SqliteFailure(
    ...
    pub fn value_ref(inner: &sqlanywhere_sys::Statement, col: usize) -> ValueRef<'_> {
