...
use sqlanywhere::{params, Connection, Database, Value};
...
fn basic_metrics() {
    ...
    sim.client("test", async {
        ...
        snapshot.assert_counter("sqlanywhere_server_sqlanywhere_execute_program", 3);
        snapshot.assert_counter("sqlanywhere_server_user_http_response", 4);
        ...
        for (key, (_, _, val)) in snapshot.snapshot() {
                ...
                && key.key().name() == "sqlanywhere_client_version"
            {
                ...
                assert!(label.value().starts_with("sqlanywhere-remote-"));
...
async fn insert_rows(conn: &Connection, start: u32, count: u32) -> sqlanywhere::Result<()> {
...
async fn assert_rows_count(conn: &Connection, expected: u32) -> sqlanywhere::Result<()> {
