...
// Safety: works as long as SqlAnywhere is compiled and set up with SERIALIZABLE threading model, which is the default.
...
impl Statement {
    ...
    pub fn interrupt(&self) {
        unsafe { crate::ffi::sqlanywhere_stmt_interrupt(self.raw_stmt) }
    }
...
fn len_as_c_int(len: usize) -> Result<c_int> {
    if len >= (c_int::MAX as usize) {
        Err(crate::Error::from(
            sqlanywhere_ffi::SQLITE_TOOBIG as std::ffi::c_int,
        ))
    } else {
        Ok(len as c_int)
    }
