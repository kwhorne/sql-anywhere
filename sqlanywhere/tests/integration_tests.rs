...
async fn prepare_invalid_sql() {
    ...
    match actual {
        sqlanywhere::Error::SqliteFailure(code, msg) => {
            assert_eq!(code, 1);
...
async fn custom_params() {
    ...
    impl IntoValue for MyValue {
        fn into_value(self) -> sqlanywhere::Result<Value> {
            match self {
    ...
    conn.execute(
        "INSERT INTO users (id, name) VALUES (?1, ?2)",
        sqlanywhere::params_from_iter(params),
    )
    .await
    .unwrap();
}
...
async fn deserialize_row() {
    ...
    let row = conn
        ...
        .unwrap()
        .unwrap();
    let data: Data = sqlanywhere::de::from_row(&row).unwrap();
...
async fn vector_fuzz_test() {
    ...
    for attempt in 0..10000 {
        ...
        let _ = conn
            .execute(
                "CREATE INDEX users_idx ON users ( sqlanywhere_vector_idx(v) );",
                (),
            )
            .await;
        // println!("CREATE INDEX users_idx ON users ( sqlanywhere_vector_idx(v) );");
        ...
        for _ in 0..operations {
            ...
            if operation == 0 {
                ...
                conn.execute(
                    ...
                    sqlanywhere::params![next_id, vector_str],
                )
                .await
                .unwrap();
                ...
                conn.execute("DELETE FROM users WHERE id = ?", sqlanywhere::params![id])
                    .await
                    .unwrap();
                alive.remove(&id);
                ...
                conn.execute(
                    ...
                    sqlanywhere::params![vector_str, id],
                )
                .await
                .unwrap();
            } else if operation == 3 {
                ...
                let result = conn
                    .query(
                        ...
                        sqlanywhere::params![vector_str, k],
                    )
                    .await
                    .unwrap();
...
async fn test_deny_authorizer() {
    ...
    let res = conn
        ...
        .await;
    assert_sqlite_error(res, sqlanywhere::ffi::SQLITE_AUTH);
...
fn assert_sqlite_error<T>(res: Result<T>, code: i32) {
    match res {
        ...
        Err(e) => {
            if let sqlanywhere::Error::SqliteFailure(c, _) = e {
