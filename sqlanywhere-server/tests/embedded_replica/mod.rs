...
fn embedded_replica() {
    ...
    sim.client("client", async move {
        ...
        let sqlanywhere::Error::RemoteSqliteFailure(code, extended_code, _) = err else {
            panic!()
        };
        ...
        for (key, (_, _, val)) in snapshot.snapshot() {
            if key.kind() == metrics_util::MetricKind::Counter
                && key.key().name() == "sqlanywhere_client_version"
            {
                assert_eq!(val, &metrics_util::debugging::DebugValue::Counter(6));
                let label = key.key().labels().next().unwrap();
                assert!(label.value().starts_with("sqlanywhere-rpc-"));
            }
        }

        snapshot.assert_counter("sqlanywhere_server_user_http_response", 6);
...
fn embedded_replica_with_encryption() {
    ...
    sim.client("client", async move {
        ...
        let db = Database::open_with_remote_sync_connector(
            ...
            TurmoilConnector,
            false,
            Some(sqlanywhere::EncryptionConfig::new(
                sqlanywhere::Cipher::Aes256Cbc,
...
fn sync_interval() {
    ...
    sim.client("client", async move {
        ...
        let path = tmp_embedded_path.join("embedded");
        let db = sqlanywhere::Builder::new_remote_replica(
            path.to_str().unwrap(),
...
fn errors_on_bad_replica() {
    ...
    sim.client("client", async move {
        ...
        sqlanywhere::Builder::new_remote_replica(
            path.to_str().unwrap(),
            "http://foo.primary:8080".to_string(),
            "".to_string(),
        )
...
fn txn_bug_issue_1283() {
    ...
    sim.client("client", async move {
        ...
        let replica = sqlanywhere::Builder::new_remote_replica(
            path.to_str().unwrap(),
            db_url.to_string(),
            String::new(),
        )
        ...
        let remote = sqlanywhere::Builder::new_remote(db_url.to_string(), String::new())
            .connector(TurmoilConnector)
            .build()
            .await
            .unwrap();
    ...
    async fn db_work(conn: sqlanywhere::Connection) -> Result<(), anyhow::Error> {
...
fn remote_replica_namespace_header_support() {
    ...
    sim.client("client", async move {
        ...
        let remote_replica = sqlanywhere::Builder::new_remote_replica(
            path.to_str().unwrap(),
            db_url.to_string(),
            String::new(),
        )
