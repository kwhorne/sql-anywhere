...
const LIB_NAME: &str = "sqlanywhere";
const BUNDLED_DIR: &str = "bundled";
const SQLITE_DIR: &str = "../sqlanywhere-sqlite3";
...
fn main() {
    ...
    if std::env::var("SQLANYWHERE_DEV").is_ok() {
        make_amalgamation();
...
pub fn build_bundled(out_dir: &str, out_path: &Path) {
    ...
    if cfg!(feature = "wasmtime-bindings") {
        cfg.flag("-DSQLANYWHERE_ENABLE_WASM_RUNTIME=1");
    }

    if cfg!(feature = "sqlanywhere-disable-checkpoint-downgrade") {
        cfg.flag("-DSQLANYWHERE_DISABLE_CHECKPOINT_DOWNGRADE=1");
    }
    if cfg!(feature = "sqlanywhere-checkpoint-only-full") {
        cfg.flag("-DSQLANYWHERE_CHECKPOINT_ONLY_FULL=1");
    }
    ...
    if let Ok(extras) = env::var("SQLANYWHEREITE3_FLAGS") {
        for extra in extras.split_whitespace() {
            if extra.starts_with("-D") || extra.starts_with("-U") {
                ...
                panic!("Don't understand {} in SQLANYWHEREITE3_FLAGS", extra);
            }
        }
    }
    println!("cargo:rerun-if-env-changed=SQLANYWHEREITE3_FLAGS");
...
fn build_multiple_ciphers(out_path: &Path) -> PathBuf {
    ...
    if cfg!(feature = "wasmtime-bindings") {
        config.define("SQLANYWHERE_ENABLE_WASM_RUNTIME", "1");
    }
    config.define("SQLANYWHERE_ENCRYPTION", "1");
...
mod bindings {
    ...
    fn generating_bundled_bindings() -> bool {
        ...
        println!("cargo:rerun-if-env-changed=SQLANYWHEREITE3_SYS_BUNDLING");
        match std::env::var("SQLANYWHEREITE3_SYS_BUNDLING") {
            Ok(v) => v != "0",
            Err(_) => false,
        }
    }
    ...
    pub fn sqlite3_cancel_auto_extension(
        ...
        if !cfg!(feature = "wasmtime-bindings") {
            bindings = bindings
                ...
                .blocklist_function("sqlanywhere_run_wasm")
                .blocklist_function("sqlanywhere_wasm_engine_new")
                .blocklist_function("sqlanywhere_compile_wasm_module")
                .blocklist_function("sqlanywhere_free_wasm_module")
                .blocklist_function("sqlanywhere_wasm_engine_free");
        }
