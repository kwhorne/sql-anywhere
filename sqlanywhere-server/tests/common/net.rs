...
use sqlanywhere_server::net::Accept;
use sqlanywhere_server::net::AddrStream;
...
impl SimServer for TestServer {
    async fn start_sim(mut self, user_api_port: usize) -> anyhow::Result<()> {
        ...
        // We need to ensure that sqlanywhere's init code runs before we do anything
        // with rusqlite in sqld. This is because sqlanywhere has saftey checks and
        ...
        // to ensure we run sqlanywhere's init code first. This DB is not actually
        // used in the test only for its run once init code.
        //
        // This does change the serialization mode for sqld but because the mode
        // that we use in sqlanywhere is safer than the sqld one it is still safe.
        let db = sqlanywhere::Database::open_in_memory().unwrap();
        db.connect().unwrap();
