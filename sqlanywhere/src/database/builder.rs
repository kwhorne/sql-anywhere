...
///
/// - `new_local`/`Local` which means a `Database` that is just a local sqlanywhere database
...
cfg_core! {
    ...
    impl Builder<Local> {
        ...
        /// that sqlanywhere uses the ffi code. By default, sqlanywhere will try to use the SERIALIZED
        ...
        ///
        /// Using this setting is very UNSAFE and you are expected to use the sqlanywhere in adherence
            ...
            #[cfg(feature = "sync")]
            {
                ...
                match sync_protocol {
                    p @ (SyncProtocol::Auto | SyncProtocol::V2) => {
                        ...
                        let prefix = if url.starts_with("sqlanywhere://") {
                            url.replacen("sqlanywhere://", "https://", 1)
                        } else {
                            url.to_string()
                        };
                        ...
                        let req = if let Some(ref remote_encryption) = remote_encryption {
                            req.header("x-elyra-encryption-key", remote_encryption.key.as_string())
                        } else {
                            req
                        };
