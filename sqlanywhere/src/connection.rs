...
/// A connection to some sqlanywhere database, this can be a remote one or a local one.
#[derive(Clone)]
...
impl Connection {
    ...
    ///
    /// ```rust,no_run
    /// # async fn run(conn: &sqlanywhere::Connection) {
    /// # use sqlanywhere::params;
    ...
    /// Check whether sqlanywhere is in `autocommit` or not.
