//! # SqlAnywhere API for Rust
//!
//! SqlAnywhere is an embeddable SQL database engine based on SQLite.
...
//! There are also SqlAnywhere language bindings of this Rust crate to other languages such as [JavaScript](), Python, Go, and C.
//!
//! ## Getting Started
//!
...
//! ## Embedded Replicas
//!
//! Embedded replica is SqlAnywhere database that's running in your application process, which keeps a local copy of a remote database.
...
//! # async fn run() {
//! use sqlanywhere::Builder;
//! use sqlanywhere::replication::Frames;
//!
...
//! ## Remote database
//!
//! It is also possible to create a sqlanywhere connection that does not open a local database but
...
//! use sqlanywhere::Builder;
//!
//! let db = Builder::new_remote("sqlanywhere://my-remote-db.com".to_string(), "my-auth-token".to_string()).build().await.unwrap();
//! let conn = db.connect().unwrap();
//! conn.execute("CREATE TABLE IF NOT EXISTS users (email TEXT)", ()).await.unwrap();
//! conn.execute("INSERT INTO users (email) VALUES ('alice@example.org')", ()).await.unwrap();
//! # }
...
//! you may not want to compile the sqlanywhere C code if you just want to make HTTP requests. Feature
//! flags may be used by including the sqlanywhere crate like:
//!
//! ```toml
//! sqlanywhere = { version = "*", default-features = false, features = ["core", "replication", "remote" ]
//! ```
//!
...
/// Convenient alias for `Result` using the `sqlanywhere::Error` type.
