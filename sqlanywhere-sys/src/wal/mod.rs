...
pub struct PageHeaders {
    inner: *mut sqlanywhere_pghdr,
}

impl PageHeaders {
    pub(crate) fn as_ptr(&self) -> *const sqlanywhere_pghdr {
        self.inner
    }

    pub(crate) fn as_mut_ptr(&mut self) -> *mut sqlanywhere_pghdr {
        self.inner
    }

    /// # Safety
    /// caller must ensure the headers list validity.
    pub unsafe fn from_raw(inner: *mut sqlanywhere_pghdr) -> Self {
        Self { inner }
    }

    pub fn iter(&self) -> PageHdrIter {
        // TODO: move SQLANYWHERE_PAGE_SIZE
