...
use sqlanywhere::{params, Database};
use sqlanywhere_server::config::UserApiConfig;
use sqlanywhere_server::hrana_proto::{Batch, BatchStep, Stmt};
...
fn test_simulate_vector_index_load_from_dump() {
    ...
    sim.client("client", async {
        ...
        conn.execute("CREATE TABLE sqlanywhere_vector_meta_shadow ( name TEXT PRIMARY KEY, metadata BLOB ) WITHOUT ROWID", ()).await?;
        conn.execute("INSERT INTO sqlanywhere_vector_meta_shadow VALUES ('t_idx', x'');", ()).await?;
        conn.execute("INSERT INTO t VALUES (vector('[1,2]')), (vector('[2,3]'));", ()).await?;
        conn.execute("CREATE INDEX t_idx ON t (sqlanywhere_vector_idx(v));", ()).await?;
...
where
    ...
    Fut: std::future::Future<Output = Result<(), sqlanywhere::Error>> + 'static,
{
    let mut sim = turmoil::Builder::new()
        .simulation_duration(Duration::from_secs(1000))
        .build();
