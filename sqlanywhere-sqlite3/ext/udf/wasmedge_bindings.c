#if defined(SQLANYWHERE_ENABLE_WASM_RUNTIME) && defined(SQLANYWHERE_ENABLE_WASM_RUNTIME_WASMEDGE)

#include "sqliteInt.h"
#include "wasm_bindings.h"
#include <wasmedge/wasmedge.h>

void sqlanywhere_run_wasm(sqlanywhere_wasm_udf_api *api, sqlite3_context *context, sqlanywhere_wasm_engine_t *engine,
    sqlanywhere_wasm_module_t *module, const char *func_name, int argc, sqlite3_value **argv) {
  ...
  for (unsigned i = 0; i < argc; ++i) {
    ...
    case SQLITE_TEXT: {
      ...
      WasmEdge_String wasmedge_func_name = WasmEdge_StringCreateByCString("sqlanywhere_malloc");
...
void sqlanywhere_free_wasm_module(void *ctx) {
...
void sqlanywhere_wasm_engine_free(sqlanywhere_wasm_engine_t *eng) {
}
...
sqlanywhere_wasm_module_t *sqlanywhere_compile_wasm_module(sqlanywhere_wasm_engine_t* engine, const char *pSrcBody, int nBody,
  ...
  return (sqlanywhere_wasm_module_t*)ctx;
}

void sqlanywhere_wasm_free_msg_buf(char *err_msg_buf) {
