...
pub fn link(linker: &mut Linker<State>) -> Result<()> {
    ...
    linker.func_wrap("sqlanywhere_host", "open_fd", open_fd)?;
    linker.func_wrap("sqlanywhere_host", "delete", delete)?;
    linker.func_wrap("sqlanywhere_host", "access", access)?;
    linker.func_wrap("sqlanywhere_host", "full_pathname", full_pathname)?;
    linker.func_wrap("sqlanywhere_host", "randomness", randomness)?;
    linker.func_wrap("sqlanywhere_host", "sleep", sleep)?;
    linker.func_wrap("sqlanywhere_host", "current_time", current_time)?;
    linker.func_wrap("sqlanywhere_host", "get_last_error", get_last_error)?;
    linker.func_wrap("sqlanywhere_host", "current_time_64", current_time_64)?;

    // IO methods required by sqlite3_io_methods
    linker.func_wrap("sqlanywhere_host", "close", close)?;
    linker.func_wrap("sqlanywhere_host", "read", read)?;
    linker.func_wrap("sqlanywhere_host", "write", write)?;
    linker.func_wrap("sqlanywhere_host", "truncate", truncate)?;
    linker.func_wrap("sqlanywhere_host", "sync", sync)?;
    linker.func_wrap("sqlanywhere_host", "file_size", file_size)?;
