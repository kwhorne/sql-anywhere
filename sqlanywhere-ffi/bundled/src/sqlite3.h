...
#endif
#ifndef SQLANYWHERE_API
# define SQLANYWHERE_API
#endif
...
#define SQLANYWHERE_VERSION        "0.2.0"
...
SQLITE_API const char *sqlanywhere_libversion(void);
...
** of type "[sqlanywhere_wal_methods] **".  This opcodes will set *X
...
** CAPI3REF: Loadable Extension Thunk
**
** A pointer to the opaque sqlanywhere_api_routines structure is passed as
** the fourth parameter to entry points of [loadable extensions].  This
** structure must be typedefed in order to work around compiler warnings
** on some platforms.
*/
typedef struct sqlanywhere_api_routines sqlanywhere_api_routines;
...
typedef struct sqlanywhere_wal_manager sqlanywhere_wal_manager;

SQLITE_API int sqlanywhere_open(
  const char *filename,   /* Database filename (UTF-8) */
  sqlite3 **ppDb,         /* OUT: SQLite db handle */
  int flags,              /* Flags */
...
SQLITE_API int sqlanywhere_open_v2(
  const char *filename,   /* Database filename (UTF-8) */
  sqlite3 **ppDb,         /* OUT: SQLite db handle */
  int flags,              /* Flags */
  const char *zVfs,       /* Name of VFS module to use, NULL for default */
  const char *zWal,       /* Name of WAL module to use */
  void* pWalMethodsData   /* User data, passed to the sqlanywhere_wal struct*/
);

SQLITE_API int sqlanywhere_open_v3(
  const char *filename,   /* Database filename (UTF-8) */
  sqlite3 **ppDb,         /* OUT: SQLite db handle */
  int flags,              /* Flags */
  const char *zVfs,       /* Name of VFS module to use, NULL for default */
  sqlanywhere_wal_manager wal_manager   /* wal_manager instance, in charge of instanciating a wal */
);
...
SQLITE_API SQLANYWHERE_API int sqlanywhere_try_initialize_wasm_func_table(sqlite3 *db);
...
SQLITE_API void sqlanywhere_stmt_interrupt(sqlite3_stmt *stmt);
...
struct sqlite3_module {
  ...
  ** Those below are for version 4 and greater. */
  ...
  void (*reserved[5])();
  // following methods are added by sqlanywhere
...
** The following interfaces are provided.
**
** ^The sqlanywhere_wal_methods_find() interface returns a pointer
...
**
** ^New WAL methods are registered with sqlanywhere_wal_methods_register().
** ^Same methods can be registered multiple times without injury.
**
** ^Unregister WAL methods with the sqlanywhere_wal_methods_unregister() interface.
*/
typedef struct sqlanywhere_wal_methods sqlanywhere_wal_methods;
...
#ifdef SQLANYWHERE_CUSTOM_PAGER_CODEC
SQLITE_API void *sqlanywhere_leak_pager(sqlite3*);
#endif
...
** </dl>
**
** [[SQLANYWHERE_STMTSTATUS_ROWS_READ]]
** [[SQLANYWHERE_STMTSTATUS_ROWS_WRITTEN]]
** <dt>SQLANYWHERE_STMTSTATUS_ROWS_READ<br>
** SQLANYWHERE_STMTSTATUS_ROWS_WRITTEN</dt>
** <dd>^SQLANYWHERE_STMTSTATUS_ROWS_READ is the number of rows read when executing
** this statement. SQLANYWHERE_STMTSTATUS_ROWS_WRITTEN value is the number of
** rows written.
*/
...
#define SQLANYWHERE_STMTSTATUS_BASE              1024
#define SQLANYWHERE_STMTSTATUS_ROWS_READ         SQLANYWHERE_STMTSTATUS_BASE + 1
#define SQLANYWHERE_STMTSTATUS_ROWS_WRITTEN      SQLANYWHERE_STMTSTATUS_BASE + 2
...
** METHOD: sqlite3
**
** ^The [sqlanywhere_close_hook()] interface registers a callback function
** that is invoked prior to closing the database connection.
** ^At most one close hook may be registered at a time on a single
** [database connection]; each call to [sqlanywhere_close_hook()] overrides
** the previous setting.
** ^The close hook is disabled by invoking [sqlanywhere_close_hook()]
** with a NULL pointer as the second parameter.
** ^The third parameter to [sqlanywhere_close_hook()] is passed through as
** the first parameter to callbacks.
*/
SQLITE_API void *sqlanywhere_close_hook(sqlite3 *db, void (*xClose)(void *pCtx, sqlite3 *db), void *arg);

/*
** CAPI3REF: Disable WAL checkpointing
** METHOD: sqlite3
**
** ^The [sqlanywhere_wal_disable_checkpoint(D)] interface disables automatic
...
*/
SQLITE_API int sqlanywhere_wal_disable_checkpoint(sqlite3 *db);

/*
** CAPI3REF: Get the number of frames in the WAL file
** METHOD: sqlite3
**
** ^The [sqlanywhere_wal_frame_count(D,P)] interface returns the number of frames
** in the WAL file for [database connection] D into *P.
*/
SQLITE_API int sqlanywhere_wal_frame_count(sqlite3*, unsigned int*);

/*
** CAPI3REF: Get a frame from the WAL file
** METHOD: sqlite3
**
** ^The [sqlanywhere_wal_get_frame(D,I,P,S)] interface extracts frame I from
...
*/
SQLITE_API int sqlanywhere_wal_get_frame(sqlite3*, unsigned int, void*, unsigned int);

/*
** CAPI3REF: Begin frame insertion into the WAL
** METHOD: sqlite3
*/
SQLITE_API int sqlanywhere_wal_insert_begin(sqlite3*);

/*
** CAPI3REF: End frame insertion into the WAL
** METHOD: sqlite3
*/
SQLITE_API int sqlanywhere_wal_insert_end(sqlite3*);

/*
** CAPI3REF: Insert a frame into the WAL
** METHOD: sqlite3
*/
SQLITE_API int sqlanywhere_wal_insert_frame(sqlite3*, unsigned int, void *, unsigned int, int *);
...
#ifndef SQLANYWHERE_PAGE_HEADER_H
#define SQLANYWHERE_PAGE_HEADER_H

typedef struct sqlite3_pcache_page sqlite3_pcache_page;
typedef struct Pager Pager;
typedef struct sqlanywhere_pghdr PgHdr;
...
typedef struct sqlanywhere_pghdr sqlanywhere_pghdr;

#endif // SQLANYWHERE_PAGE_HEADER_H
...
typedef struct sqlanywhere_wal_methods {
  ...
  ** response to a ROLLBACK TO command. */
  ...
  int (*xFrames)(wal_impl* pWal, int, sqlanywhere_pghdr *, unsigned int, int, int, int*);
...
struct sqlanywhere_wal_manager {
  /* True if the implementation relies on shared memory routines (e.g. locks) */
  int bUsesShm;

  /* Open and close a connection to a write-ahead log. */
  int (*xOpen)(wal_manager_impl* pData, sqlite3_vfs*, sqlite3_file*, int no_shm_mode, long long max_size, const char* zMainDbFileName, sqlanywhere_wal* out_wal);
...
struct sqlanywhere_wal {
    sqlanywhere_wal_methods methods; /* virtual wal methods */
    wal_impl* pData; /* methods receiver */
};

typedef struct RefCountedWalManager {
    int n;
    sqlanywhere_wal_manager ref;
    int is_static;
} RefCountedWalManager;

int make_ref_counted_wal_manager(sqlanywhere_wal_manager wal_manager, RefCountedWalManager **out);
...
SQLITE_API extern const sqlanywhere_wal_manager sqlite3_wal_manager;
...
*/
SQLITE_API void sqlanywhere_run_wasm(struct sqlanywhere_wasm_udf_api *api, sqlite3_context *context,
...
*/
SQLITE_API sqlanywhere_wasm_module_t *sqlanywhere_compile_wasm_module(sqlanywhere_wasm_engine_t* engine, const char *pSrcBody, int nBody,
    void *(*alloc_err_buf)(unsigned long long), char **err_msg_buf);

/*
** Frees a module allocated with sqlanywhere_compile_wasm_module
*/
SQLITE_API void sqlanywhere_free_wasm_module(void *module);

/*
** Creates a new wasm engine
*/
SQLITE_API sqlanywhere_wasm_engine_t *sqlanywhere_wasm_engine_new();
SQLITE_API void sqlanywhere_wasm_engine_free(sqlanywhere_wasm_engine_t *);
