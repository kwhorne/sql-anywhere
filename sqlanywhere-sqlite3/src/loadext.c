...
void sqlite3AutoLoadExtensions(sqlite3 *db){
  ...
  for(i=0; go; i++){
    ...
    const sqlanywhere_api_routines *pThunkSqlanywhere = 0;
#else
    const sqlite3_api_routines *pThunk = &sqlite3Apis;
    const sqlanywhere_api_routines *pThunkSqlanywhere = &sqlanywhereApis;
#endif
    sqlite3_mutex_enter(mutex);
    if( i>=wsdAutoext.nExt ){
      xInit = 0;
    ...
    sqlite3_mutex_leave(mutex);
    zErrmsg = 0;
    if( xInit && (rc = xInit(db, &zErrmsg, pThunk, pThunkSqlanywhere))!=0 ){
