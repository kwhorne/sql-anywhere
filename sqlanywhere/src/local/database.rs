...
impl Database {
    ...
    pub fn open<S: Into<String>>(db_path: S, flags: OpenFlags) -> Result<Database> {
        ...
        if db_path.starts_with("sqlanywhere:")
    ...
    pub async fn open_local_with_offline_writes(
        ...
        let db_path = db_path.into();
        let endpoint = if endpoint.starts_with("sqlanywhere:") {
            endpoint.replace("sqlanywhere:", "https:")
        } else {
            endpoint
        };
    ...
    pub fn new(db_path: String, flags: OpenFlags) -> Database {
        static SQLANYWHERE_INIT: Once = Once::new();

        SQLANYWHERE_INIT.call_once(|| {
            ...
            unsafe {
                assert_eq!(
                    ...
                    ffi::SQLITE_OK,
                    "sqlanywhere was configured with an incorrect threading configuration and
                    ...
                    https://github.com/sqlanywhere/sqlanywhere"
                );

                assert_eq!(
                    ffi::sqlite3_initialize(),
                    ffi::SQLITE_OK,
                    "sqlanywhere failed to initialize"
                );
            }
        });
