...
// Construct a sqlanywhere_wal instance from a pointer to a Wal. This pointer must be valid until a call
// to WalManager::close
pub(crate) fn construct_sqlanywhere_wal<W: Wal>(wal: *mut W) -> sqlanywhere_wal {
    sqlanywhere_wal {
        methods: sqlanywhere_wal_methods {
            iVersion: 1,
            xLimit: Some(limit::<W>),
...
/// Turn a `WalManager` into a `sqlanywhere_wal_manager`.
/// The caller is responsible for deallocating `sqlanywhere_wal_manager.pData`
pub fn make_wal_manager<T: WalManager>(wal_manager: T) -> sqlanywhere_wal_manager {
    sqlanywhere_wal_manager {
...
pub unsafe extern "C" fn open<T: WalManager>(
    ...
    match this.open(
        ...
        Ok(wal) => {
            ...
            *out_wal = construct_sqlanywhere_wal(wal);
            SQLITE_OK
        }
...
pub unsafe extern "C" fn undo<T: Wal>(
    ...
    impl UndoHandler for SqliteUndoHandler {
        fn handle_undo(&mut self, page_no: u32) -> Result<(), sqlanywhere_ffi::Error> {
            let rc = unsafe { (self.f)(self.data, page_no) };
            if rc != 0 {
                Err(sqlanywhere_ffi::Error::new(rc))
            } else {
                Ok(())
            }
        }
...
pub unsafe extern "C" fn db<T: Wal>(wal: *mut wal_impl, db: *mut sqlanywhere_ffi::sqlite3) {
    let this = &mut (*(wal as *mut T));
    let mut db = Sqlite3Db { inner: db };
    this.set_db(&mut db);
}
