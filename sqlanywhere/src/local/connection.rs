...
/// A connection to a SqlAnywhere database.
#[derive(Clone)]
...
impl Connection {
    ...
    /// Get a raw handle to the underlying SqlAnywhere connection
    pub fn handle(&self) -> *mut ffi::sqlite3 {
        self.raw
    }

    /// Create a connection from a raw handle to the underlying SqlAnywhere connection
    ...
    ///
    /// ```rust,no_run,ignore
    /// # use sqlanywhere::Result;
    /// # use sqlanywhere::v1::{Connection, Rows};
    /// # fn create_tables(conn: &Connection) -> Result<Option<Rows>> {
    /// conn.query("SELECT * FROM users WHERE name = ?1;", vec![sqlanywhere::Value::from(1)])
    /// # }
    /// ```
                ...
                if returned_rows {
                    ...
                    let cols = stmt
                        ...
                        .map(|(i, c)| {
                            ...
                            let t = match val {
                                sqlanywhere_sys::ffi::SQLITE_INTEGER => ValueType::Integer,
                                sqlanywhere_sys::ffi::SQLITE_FLOAT => ValueType::Real,
                                sqlanywhere_sys::ffi::SQLITE_BLOB => ValueType::Blob,
                                sqlanywhere_sys::ffi::SQLITE_TEXT => ValueType::Text,
                                sqlanywhere_sys::ffi::SQLITE_NULL => ValueType::Null,
    ...
    pub fn authorizer(&self, hook: Option<AuthHook>) -> Result<()> {
        unsafe {
            let rc =
                sqlanywhere_sys::ffi::sqlite3_set_authorizer(self.handle(), None, std::ptr::null_mut());
            if rc != ffi::SQLITE_OK {
        ...
        *self.authorizer.write() = hook.clone();
        ...
        let rc =
            unsafe { sqlanywhere_sys::ffi::sqlite3_set_authorizer(self.handle(), callback, user_data) };
        if rc != ffi::SQLITE_OK {
            return Err(crate::errors::Error::SqliteFailure(
                rc as std::ffi::c_int,
    ...
    pub(crate) fn wal_checkpoint(&self, truncate: bool) -> Result<()> {
        ...
        let checkpoint_mode = if truncate {
            sqlanywhere_sys::ffi::SQLITE_CHECKPOINT_TRUNCATE
        } else {
            sqlanywhere_sys::ffi::SQLITE_CHECKPOINT_PASSIVE
        };
        let rc = unsafe {
            sqlanywhere_sys::ffi::sqlite3_wal_checkpoint_v2(
                self.handle(),
                std::ptr::null(),
                checkpoint_mode,
                &mut pn_log,
        ...
        if rc != 0 {
            let err_msg = unsafe { sqlanywhere_sys::ffi::sqlite3_errmsg(self.handle()) };
        ...
        if truncate && (pn_log != 0 || pn_ckpt != 0) {
            return Err(crate::errors::Error::SqliteFailure(
                sqlanywhere_sys::ffi::SQLITE_ERROR,
    ...
    pub(crate) fn wal_frame_count(&self) -> u32 {
        ...
        unsafe { sqlanywhere_sys::ffi::sqlanywhere_wal_frame_count(self.handle(), &mut max_frame_no) };
    ...
    pub(crate) fn wal_get_frame(&self, frame_no: u32, page_size: u32) -> Result<bytes::BytesMut> {
        ...
        let rc = unsafe {
            sqlanywhere_sys::ffi::sqlanywhere_wal_get_frame(
                self.handle(),
                frame_no,
    ...
    fn wal_disable_checkpoint(&self) -> Result<()> {
        let rc = unsafe { sqlanywhere_sys::ffi::sqlanywhere_wal_disable_checkpoint(self.handle()) };
        if rc != 0 {
            return Err(crate::errors::Error::SqliteFailure(
                rc as std::ffi::c_int,
    ...
    fn wal_insert_begin(&self) -> Result<()> {
        let rc = unsafe { sqlanywhere_sys::ffi::sqlanywhere_wal_insert_begin(self.handle()) };
        if rc != 0 {
            return Err(crate::errors::Error::SqliteFailure(
                rc as std::ffi::c_int,
    ...
    fn wal_insert_end(&self) -> Result<()> {
        let rc = unsafe { sqlanywhere_sys::ffi::sqlanywhere_wal_insert_end(self.handle()) };
        if rc != 0 {
            return Err(crate::errors::Error::SqliteFailure(
                rc as std::ffi::c_int,
    ...
    fn wal_insert_frame(&self, frame_no: u32, frame: &[u8]) -> Result<()> {
        ...
        let rc = unsafe {
            sqlanywhere_sys::ffi::sqlanywhere_wal_insert_frame(
                self.handle(),
                frame_no,
