...
The `sqld` program provides sqlanywhere over HTTP and supports transparent replication.

![sqlanywhere cluster overview.](sqld-overview.png)

<p align="center">
Figure 1. Overview of sqlanywhere clustering.
</p>

Figure 1 shows an overview of sqlanywhere cluster where clients execute SQL remotely over HTTP against `sqld` instances.
...
In this section, we will walk you through how to set up a sqlanywhere cluster.
...
```console
docker pull ghcr.io/sqlanywhere/sqld:main
```
...
The `sqld` generates incremental snapshots of the database file, which you can apply to a local SQL Anywhere replica.
...
You can now, for example, `rsync` the snapshot file to another machine, to apply the changes to a local replica with the `Database::sync_frames()` method of the `sqlanywhere` crate:

```rust
use sqlanywhere::Database;
use sqlanywhere_replication::{Frames, TempSnapshot};

#[tokio::main]
async fn main() {
    tracing_subscriber::fmt::init();

    let opts = sqlanywhere::Opts::with_sync();
