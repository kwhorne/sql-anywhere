# Bottomless S3-compatible virtual WAL for SQL Anywhere
...
```shell
SQLANYWHERE_DIR=/path/to/your/sqlanywhere/directory make
```

will produce a loadable `.so` SQL Anywhere extension with bottomless WAL implementation.

```shell
SQLANYWHERE_DIR=/path/to/your/sqlanywhere/directory make release
```
...
Default endpoint can be overridden by an environment variable too, and in the future it will be available directly from SQL Anywhere as an URI parameter:

```shell
export SQLANYWHERE_BOTTOMLESS_ENDPOINT='http://localhost:9042'
```

Bucket used for replication can be configured with:

```shell
export SQLANYWHERE_BOTTOMLESS_BUCKET='custom-bucket'
```
...
From SQL Anywhere shell, load the extension and open a database file with `bottomless` WAL, e.g.:
...
In order to customize logging, use `RUST_LOG` env variable, e.g. `RUST_LOG=info ./sqlanywhere`.

A short demo script is in `test/smoke_test.sh`, and can be executed with:

```sh
SQLANYWHERE_DIR=/path/to/your/sqlanywhere/directory make test
```
...
```sh
cd test/
export SQLANYWHERE_BOTTOMLESS_ENDPOINT=http://localhost:9000
./smoke_test.sh
./restore_test.sh
```
