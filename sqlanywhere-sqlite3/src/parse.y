...
// as currently the syntax is only used by sqlanywhere's rust library to 
...
// the database impl of sqlanywhere.
...
cmd ::= createkw FUNCTION ifnotexists(E) nm(N) LANGUAGE nm(L) AS STRING(B). {
  sqlanywhere_create_function(pParse, &N, &L, &B, 0, E);
}

cmd ::= createkw FUNCTION ifnotexists(E) nm(N) LANGUAGE nm(L) AS BLOB(B). {
  sqlanywhere_create_function(pParse, &N, &L, &B, 1, E);
}
...
cmd ::= DROP FUNCTION ifexists(E) nm(N). {
  sqlanywhere_drop_function(pParse, &N, E);
}
...
cmd ::= ALTER TABLE fullname(X) ALTER COLUMNKW columnname(Y) TO columnname(Z) carglist. {
  ...
  sqlanywhereAlterAlterColumn(pParse, X, &Y, &Z, definitionLength);
}
