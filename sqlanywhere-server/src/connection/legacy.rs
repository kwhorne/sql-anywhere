...
#[cfg(test)]
impl LegacyConnection<sqlanywhere_sys::wal::wrapper::PassthroughWalWrapper> {
    pub async fn new_test(path: &Path) -> Self {
        ...
        Self::new(
            path.to_owned(),
            Arc::new([]),
            sqlanywhere_sys::wal::wrapper::PassthroughWalWrapper,
            Default::default(),
            Default::default(),
...
pub fn open_conn<T>(
    ...
    encryption_config: Option<EncryptionConfig>,
) -> Result<sqlanywhere_sys::Connection<InhibitCheckpoint<T::Wal>>, rusqlite::Error>
where
    T: WalManager,
{
...
pub fn open_conn_active_checkpoint<T>(
    ...
    auto_checkpoint: u32,
    encryption_config: Option<EncryptionConfig>,
) -> Result<sqlanywhere_sys::Connection<T::Wal>, rusqlite::Error>
where
    T: WalManager,
{
    ...
    sqlanywhere_sys::Connection::open(
        path.join("data"),
        flags,
        wal_manager,
        auto_checkpoint,
        ...
        let (conn, id) = tokio::task::spawn_blocking({
            ...
            move || -> crate::Result<_> {
                ...
                conn.raw().create_scalar_function(
                    "sqlanywhere_server_database_name",
                    0,
