...
pub(super) async fn make_primary_connection_maker(
    ...
    let mut is_dirty = {
        ...
        if sentinel_path.try_exists()? {
            if std::env::var("SQLANYWHERE_IGNORE_DIRTY_LOG").is_ok() {
    ...
    let bottomless_replicator = match primary_config.bottomless_replication {
        Some(ref options) => {
            ...
            if !std::env::var("SQLANYWHERE_DISABLE_INIT_CHECKPOINTING").is_ok() {
    ...
    tracing::debug!("Opening sqlanywhere connection");
    ...
    tracing::debug!("Completed opening sqlanywhere connection");
            ...
            Ok(Some(cmd)) => {
                ...
                if !skipped_wasm_table {
                    if let Cmd::Stmt(Stmt::CreateTable { tbl_name, .. }) = &cmd {
                        if tbl_name.name.0 == "sqlanywhere_wasm_func_table" {
