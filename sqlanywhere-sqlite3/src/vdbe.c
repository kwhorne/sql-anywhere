...
** (sqlanywhere_stmt_interrupt()) on the statement that is currently executing,
...
void sqlanywhere_inc_row_read(Vdbe *p, int count) {
    u32 *read = &p->aSqlanywhereCounter[SQLANYWHERE_STMTSTATUS_ROWS_READ - SQLANYWHERE_STMTSTATUS_BASE];
    *read = saturating_add(*read, count);
}

void sqlanywhere_inc_row_written(Vdbe *p, int count) {
    u32 *write = &p->aSqlanywhereCounter[SQLANYWHERE_STMTSTATUS_ROWS_WRITTEN - SQLANYWHERE_STMTSTATUS_BASE];
  ...
  for(pOp=&aOp[p->pc]; 1; pOp++){
    ...
    */
    ...
    if( sqlite3_stmt_interrupt_count>0 ){
      ...
      if( sqlite3_stmt_interrupt_count==0 ){
        sqlanywhere_stmt_interrupt((sqlite3_stmt*)p);
      }
    }
#endif
...
case OP_Count: {         /* out2 */
  ...
  if( pOp->p3 ){
    ...
    sqlanywhere_inc_row_read(p, nPages);
    if( rc ) goto abort_due_to_error;
  }
  pOut = out2Prerelease(p, pOp);
  pOut->u.i = nEntry;
...
#   define MAX_ROWID  (i64)( (((u64)0x7fffffff)<<32) | (u64)0xffffffff )
    ...
    if ( pOp->p3 == SQLANYWHERE_RANDOM_ROWID_MARKER) {
      pC->useRandomRowid = 1;
      pOp->p3 = 0;
    }
...
case OP_Insert: {
  ...
  if (!pC->isEphemeral) sqlanywhere_inc_row_written(p, 1);
...
case OP_Clear: {
  ...
  if( pOp->p3 ){
    p->nChange += nChange;
    sqlanywhere_inc_row_written(p, nChange);
    if( pOp->p3>0 ){
      assert( memIsValid(&aMem[pOp->p3]) );
      memAboutToChange(p, &aMem[pOp->p3]);
  ...
  if( ALWAYS(pModule->xUpdate) ){
    ...
    if( (rc&0xff)==SQLITE_CONSTRAINT && pOp->p4.pVtab->bConstraint ){
      if( pOp->p5==OE_Ignore ){
        ...
        sqlanywhere_inc_row_written(p, 1);
        p->nChange++;
    }
    if( rc ) goto abort_due_to_error;
  }
