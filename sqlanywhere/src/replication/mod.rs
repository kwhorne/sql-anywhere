//! Utilities used when using a replicated version of sqlanywhere.
...
pub use sqlanywhere_replication::frame::{Frame, FrameNo};
use sqlanywhere_replication::injector::SqliteInjector;
use sqlanywhere_replication::replicator::{Either, Replicator};
pub use sqlanywhere_replication::snapshot::SnapshotFile;
...
use sqlanywhere_replication::replicator::ReplicatorClient;
...
impl From<sqlanywhere_replication::replicator::Error> for errors::Error {
    fn from(err: sqlanywhere_replication::replicator::Error) -> Self {
    ...
    pub async fn sync_oneshot(&self) -> Result<Replicated> {
        ...
        loop {
            match replicator.replicate().await {
                Err(sqlanywhere_replication::replicator::Error::Meta(
                    sqlanywhere_replication::meta::Error::LogIncompatible,
                )) => {
