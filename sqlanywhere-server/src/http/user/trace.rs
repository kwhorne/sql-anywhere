...
pub(crate) fn request<B>(req: &Request<B>, span: &Span) {
    ...
    if let Some(v) = req.headers().get("x-sqlanywhere-client-version") {
...
pub(crate) fn response<B>(res: &Response<B>, latency: Duration, span: &Span) {
    ...
    if !is_grpc {
        ...
        metrics::increment_counter!(
            "sqlanywhere_server_user_http_response",
            "protocol" => "http",
            "status" => status.as_str().to_string()
        );

        if status.is_server_error() {
            metrics::increment_counter!("sqlanywhere_server_user_http_fault", "protcol" => "http");
        }
    } else {
        ...
        let code = match grpc.classify_response(res) {
            ...
            ClassifiedResponse::Ready(Err(GrpcFailureClass::Code(code))) => {
                metrics::increment_counter!("sqlanywhere_server_user_http_fault", "protcol" => "grpc");
                code.to_string()
            }
