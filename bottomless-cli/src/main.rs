...
async fn run() -> Result<()> {
    ...
    if let Some(ep) = options.endpoint.as_deref() {
        std::env::set_var("SQLANYWHERE_BOTTOMLESS_ENDPOINT", ep)
    } else {
        options.endpoint = std::env::var("SQLANYWHERE_BOTTOMLESS_ENDPOINT").ok();
    }

    if let Some(bucket) = options.bucket.as_deref() {
        std::env::set_var("SQLANYWHERE_BOTTOMLESS_BUCKET", bucket)
    } else {
        options.bucket = std::env::var("SQLANYWHERE_BOTTOMLESS_BUCKET").ok();
    }
    ...
    if let Some(encryption_key) = options.encryption_key.as_ref() {
        std::env::set_var(
            "SQLANYWHERE_BOTTOMLESS_ENCRYPTION_KEY",
    ...
    if let Some(ref db_name) = options.db_name {
        if namespace_init != "" {
            std::env::set_var(
                "SQLANYWHERE_BOTTOMLESS_DATABASE_ID",
                format!("ns-{}:{}", &namespace_init, db_name),
            );
        } else {
            return Err(anyhow!(
                "db_name can be set only if SQLANYWHERE_BOTTOMLESS_DATABASE_ID env var has namespace ID"
            ));
        }
    } else {
        let namespace = options.namespace.as_deref().unwrap_or("ns-default");
        std::env::set_var("SQLANYWHERE_BOTTOMLESS_DATABASE_ID", namespace);
    }
    let namespace = std::env::var("SQLANYWHERE_BOTTOMLESS_DATABASE_ID").unwrap();
    if namespace_init != namespace {
        tracing::info!(
            "SQLANYWHERE_BOTTOMLESS_DATABASE_ID env var were updated: '{}' -> '{}'",
            namespace_init,
            namespace
        );
    }
    match options.command {
        Commands::Create { ref source_db_path } => {
            ...
            let connection = sqlanywhere_sys::Connection::open(
