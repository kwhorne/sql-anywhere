...
pub enum Error {
    #[error("SqlAnywhere failed to bind provided query parameters: `{0}`")]
    SqlAnywhereInvalidQueryParams(anyhow::Error),
    #[error("Transaction timed-out")]
    SqlAnywhereTxTimeout,
    ...
    ReplicaMetaError(#[from] sqlanywhere_replication::meta::Error),
    #[error("Replicator error: {0}")]
    ReplicatorError(#[from] sqlanywhere_replication::replicator::Error),
...
impl IntoResponse for &Error {
    fn into_response(self) -> axum::response::Response {
        ...
        match self {
            ...
            SqlAnywhereInvalidQueryParams(_) => self.format_err(StatusCode::BAD_REQUEST),
            SqlAnywhereTxTimeout => self.format_err(StatusCode::BAD_REQUEST),
            SqlAnywhereTxBusy => self.format_err(StatusCode::TOO_MANY_REQUESTS),
            ...
            ReplicatorError(e) => match e {
                sqlanywhere_replication::replicator::Error::NamespaceDoesntExist => {
