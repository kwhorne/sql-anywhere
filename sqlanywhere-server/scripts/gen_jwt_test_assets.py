...
def update_example(name, namespaces):
    ...
    open(f"sqlanywhere-server/assets/test/auth/{name}.key", "wb").write(privkey_pem)
    open(f"sqlanywhere-server/assets/test/auth/{name}.pem", "wb").write(pubkey_pem)
    open(f"sqlanywhere-server/assets/test/auth/{name}.jwt", "wb").write(token.encode())
    open(f"sqlanywhere-server/assets/test/auth/combined123.pem", "ab").write(pubkey_pem)

open(f"sqlanywhere-server/assets/test/auth/combined123.pem", "wb").write("".encode())
