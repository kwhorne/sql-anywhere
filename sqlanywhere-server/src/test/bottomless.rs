...
async fn start_s3_server() {
    std::env::set_var("SQLANYWHERE_BOTTOMLESS_ENDPOINT", "http://localhost:9000");
    std::env::set_var("SQLANYWHERE_BOTTOMLESS_AWS_SECRET_ACCESS_KEY", "foo");
    std::env::set_var("SQLANYWHERE_BOTTOMLESS_AWS_ACCESS_KEY_ID", "bar");
    std::env::set_var("SQLANYWHERE_BOTTOMLESS_AWS_DEFAULT_REGION", "us-east-1");
    std::env::set_var("SQLANYWHERE_BOTTOMLESS_BUCKET", "my-bucket");
    ...
    S3_SERVER.call_once(|| {
        ...
        let key = std::env::var("SQLANYWHERE_BOTTOMLESS_AWS_ACCESS_KEY_ID").unwrap();
        let secret = std::env::var("SQLANYWHERE_BOTTOMLESS_AWS_SECRET_ACCESS_KEY").unwrap();
...
async fn s3_config() -> aws_sdk_s3::config::Config {
    ...
    aws_sdk_s3::config::Builder::from(&loader.load().await)
        ...
        .region(Region::new(
            std::env::var("SQLANYWHERE_BOTTOMLESS_AWS_DEFAULT_REGION").unwrap(),
        ))
        .credentials_provider(Credentials::new(
            std::env::var("SQLANYWHERE_BOTTOMLESS_AWS_ACCESS_KEY_ID").unwrap(),
            std::env::var("SQLANYWHERE_BOTTOMLESS_AWS_SECRET_ACCESS_KEY").unwrap(),
            None,
            None,
            "Static",
        ))
