...
use futures::Future;
use hyper::Uri;
use sqlanywhere_replication::rpc::replication::replication_log_client::ReplicationLogClient;
...
impl ConfigureNamespace for ReplicaConfigurator {
    ...
    fn setup<'a>(
        ...
        Box::pin(async move {
            ...
            .await?;
            let mut replicator = sqlanywhere_replication::replicator::Replicator::new_sqlite(
                client,
                db_path.join("data"),
            ...
            join_set.spawn(async move {
                use sqlanywhere_replication::replicator::Error;
                loop {
                    match replicator.run().await {
                        ...
                        Error::Meta(err) => {
                            use sqlanywhere_replication::meta::Error;
                            match err {
