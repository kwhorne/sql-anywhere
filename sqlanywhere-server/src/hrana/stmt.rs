...
pub fn stmt_error_from_sqld_error(sqld_error: SqldError) -> Result<StmtError, SqldError> {
    let result = match sqld_error {
        SqldError::SqlAnywhereInvalidQueryParams(source) => Ok(StmtError::ArgsInvalid { source }),
        SqldError::SqlAnywhereTxTimeout => Ok(StmtError::TransactionTimeout),
        SqldError::SqlAnywhereTxBusy => Ok(StmtError::TransactionBusy),
    ...
    counter!("sqlanywhere_server_hrana_step_errors", 1, "code" => code);
