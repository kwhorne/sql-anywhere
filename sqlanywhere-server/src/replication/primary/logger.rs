...
use crate::SQLANYWHERE_PAGE_SIZE;

pub use sqlanywhere_replication::FrameEncryptor;
...
impl LogFile {
    ...
    pub const FRAME_SIZE: usize = size_of::<FrameHeader>() + SQLANYWHERE_PAGE_SIZE as usize;
    ...
    pub fn new(
        ...
        let header = if file_end == 0 {
            ...
            LogFileHeader {
                ...
                page_size: (SQLANYWHERE_PAGE_SIZE as i32).into(),
        ...
        let encryption_buf = if encryption.is_some() {
            BytesMut::with_capacity(SQLANYWHERE_PAGE_SIZE as usize)
        } else {
            BytesMut::new()
        };
        let mut this = Self {
...
impl ReplicationLogger {
    ...
    fn recover(
        ...
        assert!(
            size % SQLANYWHERE_PAGE_SIZE == 0,
            "database file size is not a multiple of page size"
        );
        let num_page = size / SQLANYWHERE_PAGE_SIZE;
        let mut buf = [0; SQLANYWHERE_PAGE_SIZE as usize];
        ...
        for i in 0..num_page {
            data_file.read_exact_at(&mut buf, i * SQLANYWHERE_PAGE_SIZE)?;
...
pub fn checkpoint_db(data_path: &Path) -> anyhow::Result<()> {
    ...
    unsafe {
        ...
        conn.pragma_query(None, "page_size", |row| {
            ...
            assert_eq!(
                page_size, SQLANYWHERE_PAGE_SIZE as i32,
                "invalid database file, expected page size to be {}, but found {} instead",
                SQLANYWHERE_PAGE_SIZE, page_size
            );
            Ok(())
        })?;
